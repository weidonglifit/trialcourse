...
function loadAndAnimateSvg() {
    const container = document.getElementById('intro-svg-container');
    if (!container) return;

    // 1. 使用 fetch 讀取外部的 SVG 檔案（請確保路徑正確，例如與 html 在同一資料夾）
    fetch('weidong.svg')
        .then(response => {
            if (!response.ok) {
                throw new Error('SVG 檔案載入失敗，請檢查路徑是否正確');
            }
            return response.text(); // 將檔案內容轉為文字代碼
        })
        .then(svgCode => {
            // 2. 將讀取到的 800 多行 SVG 代碼注入到 HTML 容器中
            container.innerHTML = svgCode;

            // 3. 確保注入成功後，緊接著執行 GSAP 動畫
            initWeidongSvgAnimation(container);
        })
        .catch(error => {
            console.error('載入 SVG 時發生錯誤:', error);
        });
}

// 這是我們原本寫好的 GSAP 動畫邏輯
function initWeidongSvgAnimation(container) {
    const svgElement = container.querySelector('svg');
    if (!svgElement) return;

    // 自動抓取 SVG 內部需要動畫的部件
    let targets = svgElement.querySelectorAll('g');
    if (targets.length === 0) {
