  ...
  connectedCallback() {
    this.innerHTML = `
      <div class="notice-pulse-container">
        ...
        <svg class="glow-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" style="width: 25px; height: 25px; fill: #F4D03F;">
  ...
  for (const sheetCategory in rulesMap) {
    ...
    items.forEach(item => {
      ...
      if (item.title === "⚠️ 注意事項") {
        ...
        contentDiv.style.cssText = 'padding-left: 10px; border-left: 3px solid #E87A90; margin-top: 10px; line-height: 1.6; color: #555; letter-spacing: 1px;';
...
請立刻執行核心守則，將上方每一筆預約的教室、日期與時間用溫柔且漂亮的排版呈現給學員，千萬不要漏掉任何一筆！】`;
    ...
    try {
      ...
      var brainContext = "【💡 現實世界明天天氣預報資訊】:\n" + aiTomorrowWeatherText +
        ...
        "\n\n當顧客詢問推薦但沒有指定類型時，優先推薦報名人數是2到3人的課程(最大人數-剩餘名額)" +
