...
// 建立一個專屬函式來啟動這個特效
function enableScrollInertia() {
  const menuList = document.querySelector('.quick-menu-list');
  
  // 防呆：確保網頁上有這個清單才執行
  if (!menuList) return; 

  let isScrolling;
  let lastScrollTop = menuList.scrollTop;
  
  // 抓取清單內所有的子元素（例如裡面的 li、a 或 button）
  const items = menuList.children; 

  menuList.addEventListener('scroll', () => {
    const currentScrollTop = menuList.scrollTop;
    
    // 計算滑動的方向與速度 (現在位置 - 剛剛位置)
    const delta = currentScrollTop - lastScrollTop;
    lastScrollTop = currentScrollTop;

    // 設定位移量：
    // 往下滾 delta 是正數，但文字要「往上」所以加負號。乘上 1.5 是為了讓效果更明顯。
    let offset = -delta * 1.5; 
    
    // 設定極限值，避免使用者滑太快時，文字飛出畫面 (限制在上下 25px 內)
    if (offset > 25) offset = 25;
    if (offset < -25) offset = -25;

    // 1. 滑動當下：拔掉過渡動畫，讓元素無延遲地跟著手指/滾輪偏移
    Array.from(items).forEach(item => {
      item.style.transition = 'none';
      item.style.transform = `translateY(${offset}px)`;
    });

    // 2. 偵測滑動停止：每次觸發 scroll 就清除計時器
    clearTimeout(isScrolling);

    // 當滑動完全停止 100 毫秒後，執行「彈回原位」的動作
    isScrolling = setTimeout(() => {
      Array.from(items).forEach(item => {
        // 加上帶有「物理 Q 彈感」的 CSS 動畫，並將位置歸零
...
document.addEventListener('DOMContentLoaded', enableScrollInertia);